...
- Hero: full-bleed clinic poster (`/images/hero/clinic-hero.png`); left gradient overlay; mobile/tablet `.hero-bg-image` uses centered poster framing via `object-position` (~42–44% horizontal) so the clinic sign stays in frame; combined hours/location `.hero-info-card` on desktop.
...
- Admin SPA routes: `/dashboard`, `/queue`, `/bookings`, `/enquiries`, `/analytics`, `/treatment-options`, `/settings`, `/services`, `/testimonials`, `/results`, `/gallery`, `/hero`, `/doctor`; login via `src/lib/admin-url.ts` (`VITE_ADMIN_URL`); patient fields read-only, status/`internal_notes` editable; follow-up queue stacks call/WhatsApp/confirm actions; CMS singletons `clinic_settings` and `doctor_profile` (id=1) use `.maybeSingle()` with upsert-on-empty (missing seed rows caused 406/blank pages).
