-- Idempotent content backfill for the live project.
-- The original seed.sql was never applied to this remote database, so the admin
-- CMS pages (clinic settings, doctor profile, services, testimonials, results,
-- gallery, hero, treatment options) rendered empty or failed on `.single()`.
-- Each block only inserts when the target table has no rows, so re-running is safe.

-- Service categories (fixed UUIDs for service references)
...
-- Phones
insert into phones (number, role, label, sort_order)
select * from (values
...
-- Treatment options
insert into treatment_options (label, sort_order)
select * from (values
  ('Skin Consultation', 1),
...
where not exists (select 1 from treatment_options);

-- Services (skin, hair, aesthetic)
insert into services (category_id, title, description, benefits, result, image_url, featured, sort_order)
select * from (values
  ('11111111-1111-1111-1111-111111111101'::uuid, 'Acne & Pigmentation', 'Comprehensive medical treatment for acne, acne scars, melasma, post-acne marks, and nail problems using clinically proven protocols.', '["Reduces breakouts","Fades dark spots","Clears skin tone"]'::jsonb, 'Visibly clearer skin within 4 to 8 weeks', 'https://images.pexels.com/photos/3985361/pexels-photo-3985361.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', true, 1),
  ('11111111-1111-1111-1111-111111111101'::uuid, 'Chemical Peels', 'Medical-grade peels tailored to your skin type, targeting texture, tone, and radiance with precision-controlled exfoliation.', '["Refines texture","Brightens complexion","Minimizes pores"]'::jsonb, 'Smoother, luminous skin in 1 to 3 sessions', 'https://images.pexels.com/photos/4586728/pexels-photo-4586728.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', true, 2),
  ('11111111-1111-1111-1111-111111111101'::uuid, 'Skin Analyzer & Skin Tests', 'Advanced Skin Analyzer diagnostics and clinical skin tests to accurately assess your skin condition and build a personalized treatment roadmap.', '["Skin Analyzer scan","Accurate diagnosis","Customized plan"]'::jsonb, 'Clear treatment plan in a single consultation', 'https://images.pexels.com/photos/4586732/pexels-photo-4586732.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', true, 3),
  ('11111111-1111-1111-1111-111111111101'::uuid, 'Microneedling', 'Precision collagen induction therapy that stimulates natural skin regeneration for firmer, smoother, and more youthful-looking skin.', '["Boosts collagen","Reduces scars","Improves elasticity"]'::jsonb, 'Firmer, rejuvenated skin within 4 to 6 weeks', 'https://images.pexels.com/photos/32260064/pexels-photo-32260064.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 4),
  ('11111111-1111-1111-1111-111111111101'::uuid, 'Laser Procedures', 'Advanced laser technology for skin resurfacing, pigmentation correction, and targeted skin renewal with minimal downtime.', '["Targets pigmentation","Resurfaces skin","Long-lasting results"]'::jsonb, 'Clinically significant improvement after each session', 'https://images.pexels.com/photos/3985332/pexels-photo-3985332.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 5),
  ('11111111-1111-1111-1111-111111111101'::uuid, 'HydraFacial & Rejuvenation', 'Multi-step medical facial treatment combining deep cleansing, exfoliation, extraction, and hydration for instantly radiant skin.', '["Deep cleansing","Instant radiance","Zero downtime"]'::jsonb, 'Visibly refreshed skin after first treatment', 'https://images.pexels.com/photos/7479960/pexels-photo-7479960.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 6),
  ('11111111-1111-1111-1111-111111111102'::uuid, 'PRP Therapy', 'Platelet-Rich Plasma therapy using your own blood growth factors to stimulate dormant hair follicles and promote natural regrowth.', '["Activates follicles","Thickens hair","Natural process"]'::jsonb, 'Noticeable density improvement in 3 to 6 months', 'https://images.pexels.com/photos/36963686/pexels-photo-36963686.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 7),
  ('11111111-1111-1111-1111-111111111102'::uuid, 'GFC Therapy', 'Growth Factor Concentrate therapy, a next-generation advancement over PRP with a higher concentration of targeted growth factors.', '["Higher efficacy","Concentrated factors","Faster results"]'::jsonb, 'Enhanced hair density within 2 to 4 months', 'https://images.pexels.com/photos/29648642/pexels-photo-29648642.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 8),
  ('11111111-1111-1111-1111-111111111102'::uuid, 'Exosome Therapy', 'Cutting-edge regenerative treatment using exosomes to signal cellular repair and accelerate hair follicle regeneration at the root.', '["Regenerative medicine","Cellular renewal","Minimal sessions"]'::jsonb, 'Visible improvement within 6 to 10 weeks', 'https://images.pexels.com/photos/7320791/pexels-photo-7320791.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 9),
  ('11111111-1111-1111-1111-111111111102'::uuid, 'Hair Fall Consultation', 'Complete trichological evaluation for hair loss and hair care, including scalp health assessment, hormonal analysis review, and dietary guidance.', '["Root cause diagnosis","Scalp health","Treatment roadmap"]'::jsonb, 'Targeted treatment plan from first visit', 'https://images.pexels.com/photos/23349910/pexels-photo-23349910.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 10),
  ('11111111-1111-1111-1111-111111111102'::uuid, 'Hair Density Restoration', 'Multi-modal treatment combining medical therapy, nutritional support, and clinical procedures to restore optimal hair density.', '["Multi-modal approach","Proven protocols","Lasting results"]'::jsonb, 'Measurable density improvement in 3 to 6 months', 'https://images.pexels.com/photos/7320790/pexels-photo-7320790.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 11),
  ('11111111-1111-1111-1111-111111111103'::uuid, 'Botox', 'Precision botulinum toxin treatments for dynamic wrinkle reduction, facial contouring, and subtle natural rejuvenation.', '["Smooths wrinkles","Natural look","Quick procedure"]'::jsonb, 'Visible softening of lines within 7 to 14 days', 'https://images.pexels.com/photos/4586740/pexels-photo-4586740.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 12),
  ('11111111-1111-1111-1111-111111111103'::uuid, 'Dermal Fillers', 'Volume restoration and facial contouring using premium hyaluronic acid fillers for natural, balanced aesthetic enhancement.', '["Restores volume","Defines contours","Immediate results"]'::jsonb, 'Immediate visible enhancement, lasting 9 to 18 months', 'https://images.pexels.com/photos/32160039/pexels-photo-32160039.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 13),
  ('11111111-1111-1111-1111-111111111103'::uuid, 'Anti-Aging Procedures', 'Comprehensive rejuvenation protocols combining multiple modalities to restore youthful skin quality, tone, and texture.', '["Multi-modal","Comprehensive","Preventative"]'::jsonb, 'Progressive improvement with consistent treatment', 'https://images.pexels.com/photos/7479960/pexels-photo-7479960.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 14),
  ('11111111-1111-1111-1111-111111111103'::uuid, 'Laser Hair Reduction', 'Long-term hair reduction using safe, controlled laser energy to target hair follicles and inhibit regrowth across all body areas.', '["Permanent reduction","All skin types","Painless sessions"]'::jsonb, '70 to 90 percent reduction after complete treatment course', 'https://images.pexels.com/photos/4586726/pexels-photo-4586726.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 15),
  ('11111111-1111-1111-1111-111111111103'::uuid, 'Personalized Cosmetology', 'Bespoke aesthetic treatment plans combining evidence-based procedures for comprehensive skin, face, and body enhancement.', '["Tailored protocols","Combined therapies","Ongoing care"]'::jsonb, 'Holistic aesthetic improvement tailored to you', 'https://images.pexels.com/photos/3985332/pexels-photo-3985332.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=500&w=400', false, 16)
) as v(category_id, title, description, benefits, result, image_url, featured, sort_order)
where not exists (select 1 from services);

-- Testimonials
insert into testimonials (name, location, treatment, rating, quote, initial, sort_order)
select * from (values
...
) as v(name, location, treatment, rating, quote, initial, sort_order)
where not exists (select 1 from testimonials);

-- Results
insert into results (label, before_url, after_url, duration, category, sort_order)
select * from (values
...
) as v(label, before_url, after_url, duration, category, sort_order)
where not exists (select 1 from results);

-- Gallery
insert into gallery_items (image_url, label, tag, is_tall, sort_order)
select * from (values
...
-- Hero slides
insert into hero_slides (image_url, alt, sort_order)
select * from (values
...
-- Trust items
insert into trust_items (label, sub, sort_order)
select * from (values
...
) as v(label, sub, sort_order)
where not exists (select 1 from trust_items);

-- About stats
insert into about_stats (value, label, sort_order)
select * from (values
